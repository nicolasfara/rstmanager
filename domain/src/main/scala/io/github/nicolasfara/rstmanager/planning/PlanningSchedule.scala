 ...
 *
 * A planning window bounds the days that the planner is allowed to inspect and populate. The
 * domain treats both `start` and `end` as included in the window. Use [[PlanningWindow.create]]
 * when accepting external input so the invariant `end >= start` is checked and reported as a
 ...
 *
 * The assignment is deliberately slice-level, not task-level: a task may span multiple days and
 * the employee may change between slices. `availableHours` is the employee capacity for the
 * production day after weekly capacity and calendar overrides have been applied. `assignedHours`
 * is the portion of that availability consumed by this specific slice.
 *
 * The smart constructor checks the per-slice invariant `0 < assignedHours <= availableHours`.
 * It does not validate the employee's total load across all slices on the same day; that belongs
 * to the future scheduling algorithm.
 *
 * @param employeeId
 ...
 *
 * A slice is the smallest planning output currently modeled. It says that, on a given day, one
 * employee should spend `candidateEmployee.assignedHours` on one scheduled task belonging to one
 * scheduled manufacturing and order. Task effort remains hour-based through [[TaskHours]], so a
 * task that needs more work than a day allows is represented by multiple slices over multiple
 * days.
 *
 * `remainingHoursAfterSlice` records the projected task remainder after this slice is applied. It
 * is informational domain output for users and audit trails; actual task progress remains owned
 * by the work execution model.
 *
 * @param orderId
 ...
 *
 * A daily schedule groups all task slices assigned to the same production day. Use
 * [[DailySchedule.create]] when building a schedule from planner output so the model rejects empty
 * days and slices that point to a different day. Empty production days should simply be omitted
 * from a [[PlanningResult]].
 *
 * @param day
 ...
 *
 * A delayed manufacturing means the manufacturing does not fit the current planning constraints
 ...
 *
 * Warnings should be shown to users or kept for audit when planning succeeds but some condition is
 * noteworthy, such as tight capacity or a lower-priority order moving close to its deadline.
 */
 ...
 *
 * A planning result is produced only when at least one production day has planned work. Delays and
 * warnings are explicit side information derived from the same planning run. This type does not
 * mutate orders or task progress by itself; it describes the schedule that was computed.
 *
 * @param schedules
