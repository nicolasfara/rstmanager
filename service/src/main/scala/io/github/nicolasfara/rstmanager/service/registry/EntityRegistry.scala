 ...
 *
 * A single instance of this aggregate lives per entity type (one stream namespace each) and powers the collection `GET`
 * endpoints, since edomata reads one aggregate at a time and has no native "list all streams" query.
 */
